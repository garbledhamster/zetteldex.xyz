...
collapseAllBtn.addEventListener("click",()=>{
  setAllExpansion(indexList,"false");
});
expandAllBtn.addEventListener("click",()=>{
  setAllExpansion(indexList,"true");
});

function setAllExpansion(u,e){
  u.querySelectorAll("li").forEach(li=>{
...
function parseIndex(str){
  ...
  if(mainPart.length<4){
    let padded=mainPart.padEnd(4,"0");
    mainPart=padded;
  }
  if(mainPart.length>4){ 
    // in case user typed 5+ digits at start
